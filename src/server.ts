...
${state.memories ? state.memories.join("\\n") : "None yet."}
...
If the user asks to schedule a task, use the scheduleTask tool. If they ask you to remember something, use the rememberFact tool.`,
      ...
      tools: {
        ...
        // Memory tool: store user preferences and facts
        rememberFact: tool({
          description:
            "Remember a fact about the user. Call this tool when the user states a preference, a name, or a rule. You'll have access to this fact in future conversations.",
          inputSchema: z.object({
            fact: z
              .string()
              .describe(
                "The fact to remember. e.g. 'The user likes cats' or 'The user's name is John'."
              )
          }),
          execute: async ({ fact }) => {
            const state = this.state as { memories?: string[] };
            const currentMemories = state.memories || [];
            this.setState({ memories: [...currentMemories, fact] });
